...
async function downloadAsset(slug, kind, sourceUrl) {
  ...
  if (!response.ok) {
    throw new Error(`Asset download failed ${response.status} for ${sourceUrl}`)
  }
...
async function syncAssets() {
  if (!NOTION_TOKEN || !DATABASE_ID) {
    console.log('[sync-notion-assets] Missing Notion env vars, skipping asset sync.')
    return
  }
  ...
  for (const page of pages) {
    ...
    for (const block of blocks) {
      ...
      await downloadAsset(slug, `inline-${String(imageIndex).padStart(2, '0')}`, imageUrl)
      downloadCount += 1
      imageIndex += 1
    }
  }
  ...
  await writeFile(
    markerPath,
    JSON.stringify({ syncedAt: new Date().toISOString(), downloads: downloadCount }, null, 2),
  )

  console.log(`[sync-notion-assets] Downloaded ${downloadCount} asset(s) into ${OUTPUT_DIR}`)
}
